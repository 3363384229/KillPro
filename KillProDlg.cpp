	...
	while (true) {
		...
		if (mProc.type == 0) {
			...
			for (const HWND& targetHWnd : vecHWnds)
			{
				...
				if (std::regex_match(szWindowTitle, std::regex(mProc.Title))) {
	...
	while (true) {
		...
		if (mProc[i].type == 0) {
			...
			for (const HWND& targetHWnd : vecHWnds)
			{
				char szWindowTitle[64];
				GetWindowText(targetHWnd, szWindowTitle, sizeof(szWindowTitle));
				if (std::regex_match(szWindowTitle, std::regex(mProc[i].Title))) {
					ShowWindow(targetHWnd, SW_HIDE);
				}
			}
		}
...
void CKillProDlg::ReadFromJsonFile(const char* jsonFile) {//从json文件读取进程名信息
	...
	if (!reader.parse(ifs, root)) {//读取失败或格式错误
		AfxMessageBox(!hidewindow ? "数据文件读取失败，将自动创建！" : "Error:数据文件为空，无法执行后台工作命令！");
...
bool CKillProDlg::CheckStartUp() {
	TCHAR pathstr[MAX_PATH] = { 0 };
	CString strDataPath, strExecComm;
	...
	strDataPath = pathstr;
	strDataPath = strDataPath.Left(strDataPath.ReverseFind('\\') + 1);
	strExecComm.Format("\"%s\" -f \"%s\\ProcessNames.json\"--nogui", pathstr, strDataPath);
	ret = RegGetValue(HKEY_CURRENT_USER, "Software\\Microsoft\\Windows\\CurrentVersion\\Run", "KillPro", RRF_RT_REG_SZ, &dwType, content, &size);
	return ret == ERROR_SUCCESS && (CString)content == strExecComm;
}
...
bool CKillProDlg::SetStartUp(bool isStartUp)
{
	...
	if (isStartUp) {
		...
		strDataPath = strDataPath.Left(strDataPath.ReverseFind('\\') + 1);
		strExecComm.Format("\"%s\" -f \"%s\\ProcessNames.json\"--nogui", pathstr, strDataPath);
		int len = _tcslen(strExecComm);
		return RegSetKeyValue(hKey, 0, "KillPro", REG_SZ, strExecComm, len) == ERROR_SUCCESS;
	}
	else {
		return RegDeleteValue(hKey, "KillPro") == ERROR_SUCCESS;
	}
}
	...
	while (true)
	{
		...
		if (GetCursorPos(&pNow)) // 获取鼠标当前位置
		{
			...
			if (hwndPointNow)
			{
				...
				p_Dlg->SendMessage(WM_ADDNAME, 0, 0);
				Sleep(1000);
				::SetWindowPos(p_Dlg->m_hWnd, HWND_NOTOPMOST, 0, 0, 0, 0, SWP_NOMOVE | SWP_NOSIZE);
				return 0;
			}
		}
		Sleep(500);
...
void CKillProDlg::OnBnClickedButton2()//“手动选择”实现函数
{
	...
	CreateThread(NULL, 0, AutoGetProcInfo, this, 0, 0);
